...
- **Why only the boss attacked, and the levers that answer it.** The Director, playing 0.11.1 with the two-minute beds: enemies rarely attack, only the boss does, and a round stops dead at its scene. Measured in the sim with a player who holds fire and sweeps, as a player does: eighteen of a round's twenty-seven sprites died on their entry paths, six ever reached the hover, and the formation fired zero to six shots in a hundred-and-five-second seat round, because a formation sprite could fire only from the hover, only two of the five spawn classes fired at all, and the first shot waited a whole period. The headless sim was byte-identical before and after the health pass and the levers had not moved since the 2026-09-11 tune; the game had been this quiet since then. Three levers in `fire.json` now name what fires (`shooters`), whether a sprite fires on its way in (`onEntry`) and when its first shot comes (`firstShot`), and `ladder.json` may cap the ship's shots in flight (`shotsInFlight`), the Galaga rule that makes the aim matter. Each is optional and reads as before when absent. The Director played the first two tunes, called both too thin, and took the question to Grok (`docs/ghost-attack.grok-consult.md`), whose answer is what shipped: three line defects fixed, the column capped, no new guns. A sprite spawned half above the top edge could be killed before it was on the field; a shooter's clock started on its first eligible tick rather than at its entry, so a sprite paid the whole path and then a period more; and a diving grid dropped its gun, so the one class that both shoots and dives did neither as a shooter. With those fixed, the ship holds three shots in the air at seat and two at live (Galaga's rule, so the aim matters and a formation finishes its entry), the first shot comes at four tenths of a period, and the shooters stay the grid and the menu: the handshake classes are the clean stretch's flyby (G7). Measured under a player who holds fire and sweeps, fourteen or fifteen of a round's sprites now reach the hover where six did, and the round is played out. The fairness band's seat reader bar is split into coverage (every lie, as before) and survival (ended by time on half the seated tapes), and the reader bot keeps its gun through a dodge and no longer walks back under a crossing, which is a person's ceiling under fire rather than a turret under its target; the two live mover bars are lowered a step with the reason beside them, because the mover never dodges and the cap slows its hunt. The play now flows into the next tape by itself after the end scene has held (`NEXT_TAPE_S`, seven seconds; a click inside the hold replays the tape, the Next button skips the wait), in a shift as on the menu.
