...
- **A boss bed starts past its silent intro.** The Director: the boss music was mostly silent and not daunting. Measured, the boss beds of the music pass open on seven to ten seconds under thirty decibels below full scale, and carry thin stretches inside, because their intros and breakdowns were built from a quiet layer of stems that on these three beds is close to nothing. A bed may now name an `entry`, the second it starts from when adopted (`BED_ENTRY_S`: whisperer eight, menu ten, doorman seven and a half; the wave beds start at the top). The three boss beds are being rebuilt loop-first with no thin sections and mastered above the wave beds, for the Director's ear before they ship.
- **A bed plays through whole.** The Director, on the mastered two-minute beds: they sound right and end after thirty or forty seconds. A long piece had been holding a verse (`BED_MIN_S`) before a wave change could take its named bed. His decision: a bed holds for its whole length before a wave change may move it, and only a boss changes the music mid-piece. `BED_SHORT_S` is gone.
