...
- **The formation sweeps the whole field.** The Director: the enemies never went to the far left or right, so a player parked at an edge was safe from everything but a dive. The hover was a six-pixel wobble about a sprite's home; it is now a slow pass across the field, Galaxian's drift, its width and period levers per rung in `ladder.json` (`sweep`, `sweepPeriod`: nine tenths of the field every nine seconds at seat, the whole field every seven at live and every five at hardcore, half the field every twelve on the recorded rung), the phase taken from the sprite's home so a formation keeps its spacing.
