...
一个 **录音带** 是一段对话的录制。这款游戏只会读取录音带。它不会与服务器通信，也不会记录得分，也不会告诉你谁赢了。

- **三个灯。** Boss 的攻击或编队移动会熄灭一个灯。抓住一个从被击落的 Boss 处 **垂直落下** 的灯，可以重新点亮一个。你必须移动到灯的下方。全部熄灭将结束回合。
- **散布。** 清除一个编队，一个散布物会垂直落下。抓住它，你的火力将在几秒钟内增强。
- **Boss 是实验，而不是指控。** “耳语者”、“菜单”和“门卫”无论是否发生了错误，都会出现在各自的波次中。它们像一个代理一样自言自语。硬核模式只有一个灯，并且从第一次攻击开始就会产生强烈的反应。普通模式、实时模式和硬核模式都会产生短暂的 **平行** 效果，从而增加游戏场景的复杂性和音乐的节奏，并且随着回合的进行，效果会持续更长时间。在本地，你可以让一个 Ollama 模型（包括带有 `gpt-oss:120b-cloud` 这样的 Cloud 标签）来控制 Boss 的攻击。它永远不会知道哪些图像是虚假的。
- **线索在于序列。** 虚假的信息，无论从外观、移动方式还是出现时间上，都不会与真实的信息有所不同。真正揭示其身份的是它所处的位置：一个额外的编队、一个额外的菜单，或者在菜单之后立即出现。
- **结局场景** 会显示录音带的名称、服务器和策略。被捕获的虚假信息会作为战利品展示。逃脱的虚假信息会以其真实的形式呈现。没有得分，没有计数，没有数字。
