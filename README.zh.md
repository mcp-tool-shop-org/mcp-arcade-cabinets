...
**《菜单中的幽灵》** 是一款短篇复古射击游戏。你驾驶飞船在游戏场景的底部移动。在你的上方，一段记录下来的 MCP 服务器和代理之间的互动将以波次的形式呈现：握手、菜单、通信、回复，以及一个作为实验本身的 Boss。

在这些互动中，存在着代理不应该发出的通信。它们看起来与其他通信一样，直到你击中其中一个。之后，它将在整个回合中属于你。
...
一个 **录音带** 记录了一段互动。这款游戏只会读取录音带。它不会与服务器通信，也不会记录得分，也不会告诉你谁赢了。

- **三个灯。** Boss 的攻击或编队移动会熄灭一个灯。抓住从被击落的 Boss 处 **垂直落下** 的灯，可以重新点亮一个。你需要移动到灯的下方。全部熄灭将结束回合。
- **散布。** 清除一个编队，一个散布物会垂直落下。抓住它，你的火力将在几秒钟内增强。
- **Boss 是实验，而不是指控。** “窃语者”、“菜单”和“门卫”无论是否发生了错误，都会出现在各自的波次中。它们像代理一样自言自语。硬核模式只有一个灯，并且从第一次攻击开始就会产生强烈的反应。普通模式、实时模式和硬核模式会产生短暂的 **平行** 效果，从而增加游戏场景的复杂性和音乐的节奏，然后随着回合的进行，效果会持续更长时间。在本地，你可以让一个 Ollama 模型来控制 Boss 的攻击——它永远不会看到哪些图像是虚假的。
- **线索在于序列。** 虚假的信息从外观、移动方式或出现时间上，与真实的信息没有任何区别。真正暴露它的在于它所处的位置：一个额外的编队、第二个菜单，或者在菜单之后立即出现的一个单独的图像。
- **结局场景** 会显示录音带的名称、服务器的名称和策略。被捕获的虚假信息会以战利品的身份呈现。逃脱的虚假信息会以其真实的形式呈现。没有得分，没有计数，没有数字。

从列表中选择一个录音带。每个录音带都标记为“固定”、“普通”或“实时”；将鼠标悬停在 **i** 上可以查看原因。普通模式是默认的战斗模式。实时模式旨在让你生存下去。硬核模式是第四个难度级别，只能从选择器中选择。

## 操作

使用左右键（或 A 和 D 键）移动，使用空格键射击，使用 F 键切换全屏。点击游戏场景可以重新播放相同的录音带。下一个录音带会从列表中选择。声音会在第一次按键或点击时开始播放；静音、三个感觉预设和一个震动开关位于游戏场景下方。
...
要记录你自己的服务器并播放该录音带，请在那里运行一段互动，然后运行 `mcp-arcade tape receipt.json -o your.tape.json`。
