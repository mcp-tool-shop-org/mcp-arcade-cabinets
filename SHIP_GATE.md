...
- [x] `[all]` Version in manifest matches git tag — 2026-09-11: 0.6.0 in every package.json and SERVER_VERSION; tag v0.6.0 cut at release — executed by `npx @mcptoolshop/shipcheck manifest` (D2: manifest version not behind the newest released tag; `--expect <ver>` for a strict release-time match)
