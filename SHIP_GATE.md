...
- [x] `[all]` Version in manifest matches git tag — 2026-09-16: 0.10.0 in every package.json (eight) and SERVER_VERSION; tag v0.10.0; release.yml refuses a mismatch — executed by `npx @mcptoolshop/shipcheck manifest`
...
- [x] `[npm]` 2026-09-15: Gate L PASSED — `release.yml` publishes with OIDC + `--provenance`, no `NPM_TOKEN` in the repo; confirmed on the registry at 0.9.0 (`dist.attestations.url` present, `_npmUser` GitHub Actions) — Published via OIDC trusted publishing with `--provenance` — executed by `npx @mcptoolshop/shipcheck ci` (config/intent; `--registry <pkg>` also confirms the attestation on npm)
