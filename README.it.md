...
Ogni cabinet è un pacchetto a sé stante. Ogni comando avvia il gioco corrispondente su `127.0.0.1` e lo apre. Ghost’s `--mcp` lo esegue come server MCP tramite stdio; gli strumenti di container di Vibe Typer sono nella versione 4, quindi il suo `--mcp` lo indica e termina l’esecuzione. È richiesta la versione 22 o successiva di Node. Questi due pacchetti sono gli unici disponibili su npm; tutti gli altri pacchetti presenti sono privati e il passaggio tra i due cabinet avviene solo durante la fase di build di Pages.
