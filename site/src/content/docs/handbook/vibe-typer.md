...
With the host-side voice worker running (`pnpm voice`, the same worker that speaks Ghost's bosses), a **Voice** box appears in the controls row of the local game and the user says his lines out loud: the ask, the one-more-thing, the check-in, the reaction and the review. Never the agent's lines, which you type, and never the meeting chatter. The preset, rate and loudness are one lever (`patterns/cabinet.json → voice`) and the voice sheet names the same preset, so the writing model and the player are given the same person. Every take is heard back and receipted by fx-dub before it plays; a failed receipt is never played. A take that misses its beat waits for the next ask, and at a request boundary the reaction is heard before the ask that follows it. The bed ducks under a take; the keystroke samples do not, because the keystroke is the score's voice. The published page has no worker and no box. `pnpm sit --cabinet vibe-typer --voice on` measures the receipts in wall-clock time.
