...
| `drops.json`      | What a downed boss and a cleared formation drop, how fast it falls and drifts, the catch box, how long each fire drop lasts, and the draw weight among the three fire drops (spread, rapid with its own cap and cooldown, pierce) |
...
- **`fire.json` → tiers.N.formation** `period`, `burst`, `spread`, `speed`: how often a formation fires straight down, how many shots, how wide, how fast. Null at tier 0 by schema. Four more, each optional: `shooters` names the spawn classes that fire (grid and menu when absent); `onEntry` lets a sprite fire on its way in once it is on the field, not only from the hover (off when absent); `aim` points the shot at the ship rather than straight down (off when absent; on at every rung that fires since 2026-09-17, so an edge is not a quiet corner); `fromAbove` is how far above the ship's row a sprite must be before it fires at all (zero when absent; a quarter of the field at every rung that fires, so a sprite returning from the bottom or swooping low never fires from beside the ship, where a shot cannot be dodged); `firstShot` is when a sprite's first shot comes, as a fraction of the period (one when absent, which is a whole period of silence before the first shot). Measured on 2026-09-17 with the shipped values: a player holding fire killed eighteen of twenty-seven sprites on their entry paths and the formation fired zero to six shots in a seat round, so only the boss ever attacked. Set on 2026-09-17 by the Director's play: every class fires at every rung above the recorded one, on the way in, with an early first shot; hardcore fires every 1.6 seconds with wider spread and a faster boss and dive. The fairness band's bars for those rungs were re-based to what its bots measure at that heat, and say so beside the bar.
- **`ladder.json` → rungs[].sweep, sweepPeriod** how far a hovering formation swings across the field as one block, as a fraction of the room on either side of it (zero is a six-pixel wobble about its home), and how long one full swing takes. Set on 2026-09-17 so the outer sprites reach the edges and the spacing holds.
- **`ladder.json` → rungs[].shotsInFlight** how many of the ship's shots may be in the air at once; absent or null for no cap. Eight shots a second at a speed that crosses the field in under a second is a wall of six; Galaga's rule is two. A cap makes the aim matter and gives a formation time to enter.
