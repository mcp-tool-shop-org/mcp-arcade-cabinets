...
- **`fire.json` → tiers.N.formation** `period`, `burst`, `spread`, `speed`: how often a formation fires straight down, how many shots, how wide, how fast. Null at tier 0 by schema. Three more, each optional: `shooters` names the spawn classes that fire (grid and menu when absent); `onEntry` lets a sprite fire on its way in once it is on the field, not only from the hover (off when absent); `firstShot` is when a sprite's first shot comes, as a fraction of the period (one when absent, which is a whole period of silence before the first shot). Measured on 2026-09-17 with the shipped values: a player holding fire killed eighteen of twenty-seven sprites on their entry paths and the formation fired zero to six shots in a seat round, so only the boss ever attacked. Set on 2026-09-17 after a consult: the shooters stay the grid and the menu, from the hover, the first shot at four tenths of a period; the ship holds three shots in the air at seat and two at live; hardcore keeps the 09-11 numbers. The change that mattered was three line defects in the sim (a sprite hittable before it was on the field, a fire clock armed a period late, a diver that dropped its gun), and the fairness band's seat reader bar was split into coverage and survival the same day.
