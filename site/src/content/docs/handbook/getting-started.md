...
| Ollama bosses                | Local daemon or Ollama Cloud. The model sits in the boss through the cabinet server's own tools: it calls each boss's shots, may queue the next few legal verbs off the beat, and writes the boss's lines behind a gate (twelve words, one sentence, no digit, no fact word, no tool or model name; a refused line plays one of the boss's own). A hung answer is the script. It never sees lies. Pages omits this chrome. |
...
The music is a playlist: a piece runs from its first bar to its last, the next is drawn without repeats until the set is spent, and no event on the field replaces one, not a wave, not a boss, not the scene between two tapes. Beds sit under the shots and the catch. There is no burst track; a burst speeds the playing piece up and lets it back down.
...
A shift is a run of four tapes as one agent session: the rig has a task list and you are the one it sends. The draw is seeded from the clock, never from anything on a tape, and it stays fresh against the last two shifts your browser took. Each call is a different room (pressure, area-deny, trough, peak), authored in `patterns/shift.json` beside the climb list. Between calls a card names the next tape, its server, its policy, the tools the agent was asked to run, and the fight in words. The difficulty is the shift's, fixed by its code. The climb is still heat data (`copiesShift` and `intensityShift` in `parallelism.json` are the reach the last call ends on): the last call's bursts start where a tape played alone would end. Measured on the band: as the last call at live, the scripted mover dies on nine tapes of twenty against five alone, and still survives half the roster with half the lies found. Picker-alone inspect has no Archivist; a shift inspect does.
