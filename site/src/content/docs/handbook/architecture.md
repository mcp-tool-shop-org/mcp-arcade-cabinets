...
- **`cues`** diffs two snapshots of the state and names the sounds to fire; **`audio`** is pure note data until the shell attaches a WebAudio context. Recorded beds overlay the chiptune when the files are present, as a playlist: a piece runs to its end and the next is drawn from a bag the shell keeps.
- **Voice lines** are authored in `voice.json` and gated at load. Wave cards, spawn lines, catches and ends pick by seed. Asides walk a large offline bag without replacement so a round does not keep repeating the same joke. They are not an Ollama seat.
