...
Chaque module est un ensemble distinct. Chaque commande lance le jeu correspondant sur `127.0.0.1` et l’ouvre, et le `--mcp` de chaque module exécute ce module en tant que serveur MCP via stdio : les six outils de Ghost placent un modèle dans le rôle du boss ; les quatre outils de Vibe Typer (`view`, `product`, `ask`, `react`) placent n’importe quel client MCP dans le rôle de l’utilisateur pour une exécution sans fin. Version 22 ou ultérieure. Ces deux modules sont les seuls disponibles sur npm ; tous les autres modules sont privés, et le passage de l’un à l’autre ne se fait que lors de la création des pages.
