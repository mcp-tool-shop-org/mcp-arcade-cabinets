...
Cada módulo es un paquete independiente. Cada comando ejecuta su juego en `127.0.0.1` y lo abre, y el `--mcp` de cada uno ejecuta ese módulo como un servidor MCP a través de stdio: las seis herramientas de Ghost colocan un modelo en el lugar del jefe; las cuatro de Vibe Typer (`view`, `product`, `ask`, `react`) colocan cualquier cliente MCP en el lugar del usuario para una ejecución continua. Node 22 o una versión más reciente. Estos dos paquetes son los únicos que se encuentran en npm; todos los demás paquetes aquí son privados, y el cambio entre los dos módulos solo se realiza durante la compilación de Pages.
