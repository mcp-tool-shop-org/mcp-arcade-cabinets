...
## What was set after the consult

Grok's answer, in its order, is what shipped on `cabinet/attack-flow`: the three line defects (a hull hittable above the field, `hittable` now requires `y >= 0`; the fire clock armed at `tEnter` rather than on the first eligible tick; a diver keeps its gun), `shotsInFlight` 3 at seat and 2 at live, `firstShot` 0.4 at seat and live, the shooters still grid and menu, `onEntry` off. The reader bot keeps its gun through a dodge, does not walk back onto a predicted crossing, and uses the all-lanes dodge only under more than one threat in reach; the seat reader bar is split into coverage (every lie, exact, and it passes on all twenty seated tapes) and survival (ended by time on half the seated roster; measured thirteen of twenty). The live mover bars are lowered a step with a note, as the consult allowed, because the mover never dodges (measured ten of twenty alone, eight at the top of the climb). Measured at seat under a player who holds fire and sweeps: fourteen or fifteen sprites reach the hover where six did, the round plays out, and the formation's shots are still few against that player, which is the consult's point about the classes and the Director's to feel. A hotter tune (every class, on entry) is kept built beside it for that comparison and is not shipped.
