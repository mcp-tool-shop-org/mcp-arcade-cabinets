...
**G14. Gated generation, not a script (Director's word, 2026-09-10).** The research argued for authored lines (finding 8; advisory F21). The Director overrode it: "authored words will get old fast; no point in having an LLM if it stays to a script; the randomness goes with this game uniquely." So the model writes the boss's line at play time, and a gate keeps the evidence's warning in check. Parts stay scripted: the wave card, the end scene, and the fallback. The gate is four things, all tested. **Fact-blind in**: the prompt is the same words the seat gets today (kind, health, ship, stick, motion) plus the wave kind, the boss's persona sheet, three authored lines as register seeds, and the round's last few spoken lines so it does not repeat; never a fact, a lie flag, a count, a score, or a tape row. **Bounded out**: at most twelve words, one sentence, no digit, no fact or score word (the same FORBIDDEN test the prompts already throw on), no tool or model name; a line that fails the gate is dropped and the seed's authored line plays, so the scripted floor never leaves. **Relevant by construction**: what the model may react to is exactly what the prompt carries (the stick, the column, the health word, the phase, the last verb), which is also all it can see. **Receipted**: every voiced line gets an fx-dub spoken-content receipt, so the speaker said the gated text and nothing else. Timing stays the model's lever (finding 9); self-aware lines that own the boss being a model are in register (finding 10); the whole shape is bounded generation with engine validation (finding 11). (Findings 9, 10, 11; contrastive to 8 and advisory F21, decided by the Director.)
