...
The Catalog, as of 0.12.0: the registry PR ([docker/mcp-registry#5061](https://github.com/docker/mcp-registry/pull/5061)) has had no review from Docker since it was opened on 2026-09-11, so nothing is on the Docker MCP Catalog yet; its branch carried an older pin and an older `tools.json` than this repo's `catalog/`, and on 2026-09-17 it was brought to the 0.12.0 release commit with this repo's listing (silent, one parameter for an operator's tape directory, every argument described). The listing goes live when Docker reviews and merges; that is theirs. The typing cabinet's listing was opened the same evening as [docker/mcp-registry#5141](https://github.com/docker/mcp-registry/pull/5141) (`catalog/server.vibe.yaml` and `catalog/tools.vibe.json`, the same image with `CABINET=vibe` fixed in the listing's env, no parameter, network off, its own icon cut from the README logo in the brand repo), after the Director asked why it was not already there: a distribution surface is a release step, never a question. **At every release, after the tag:** copy `catalog/server.yaml` and `catalog/tools.json` over `servers/mcp-arcade-cabinets/` and `catalog/server.vibe.yaml` and `catalog/tools.vibe.json` over `servers/vibe-typer/` in the fork `mcp-tool-shop-org/mcp-registry` (`E:/AI/mcp-registry`, branches `mcp-arcade-cabinets` and `vibe-typer`), with `source.commit` at the release commit, and push each branch so its PR is current. The registry's validator shells out to `npx`, so on this rig it runs in a container with both Go and Node (`docker run --rm -v E:/AI/mcp-registry:/w -w /w golang:1.24 sh -c "apt-get install -y -qq nodejs npm && go run ./cmd/validate --name <name>"`); every check passes on both listings.
