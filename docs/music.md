...
**The menu went back the same hour.** Heard as a piece in the playlist, the loop version of the menu (a thirty-five-second loop repeated three and a half times, mastered two LU hot) read as repetitive and harsh. It is back to its route M1 arrangement at the set's level, with its receipts' rows; the whisperer and the doorman keep their loop versions. He chose none: the menu piece is out of the pool, and more beds are the next dogfood swarm's. `TRACK_KEYS` and the pack gate name seven; the Ghost table above stays as the record of what the pass made, and the menu's files stay under `docs/art/originals-ghost-beds/` (the arrangement, the loop version in git history at `8477030`, and the fresh route M2 piece `fresh/menu-8307.flac`, words gate clean) for that swarm to draw on.

## The generated playlist (2026-09-17, the Director's fifth listen)

He heard large gaps of silence inside the pieces, and the measurement (RMS per half second, the number that should have been taken on every bed before any of them shipped) agreed: the route M1 arrangements were silent for four to seven seconds at each seam and, on the poison bed, for more than half the piece; the route M2 pieces from the pass were silent from about half a minute in to the end, every one of them; and the recorded originals themselves carry silence inside their loops (poison's thirty-six seconds are half silence), so no arrangement of that material could have played through. Loudness was measured on every bed and continuity on none, which is the lapse: an integrated loudness reading gates silence out and says nothing about it.

The playlist is generated now. Seven pieces from ACE-Step 1.5 (the 4B language-model variant) on Comfy Cloud, `audio_ace_step_1_5_split_4b`, one prompt each in the arcade's register (chiptune and synthwave, instrumental, the lyric field set to `[Instrumental]`, a tempo and a key per piece, one hundred and twenty seconds asked), about twenty-two GPU seconds a piece on an RTX Pro 6000 and no API credit. Eleven were generated for seven slots; each was measured for continuity before anything else, and four were left out (a sparse opening, a silent tail of ten or twelve seconds, one that fell silent at sixty-three seconds). The seven chosen were trimmed to their music (a silent lead-in of at most three seconds and the decoder's silent tail, a thirty-millisecond fade in and a second and a half out), mastered to the set's level (-12.25 LUFS integrated, -1 dBTP; the two former boss beds no longer sit above the set, since a playlist wants one level), encoded at the pass's rung and installed. Measured on the installed files: no half-second under -35 dBFS in any of the seven, every floor above -35, lengths 114 to 115.5 seconds, loudness within a third of a decibel across the set. The words gate transcribes nothing or a caption on six and one outro caption on the seventh, the class the gate's redefinition names, and the list now names that caption. The prompts, seeds, job ids and trims are in `docs/art/originals-ghost-beds/generated/receipt.json`, beside the seven generated files; the failed route M2 pieces are gone from the tree; the originals and stems stay as history.

The slot names (`inspect`, `poison`, `rug`, `unlisted`, `breather`, `whisperer`, `doorman`) are file names now and nothing more: under the playlist rule a piece is not a wave's or a boss's. A between-rounds gap was closed the same hour: the round clock stops at the scene and nothing ticked the audio, so a piece that ended during the scene left silence until the next round's first tick; the shell now asks the audio to draw the next piece during the scene (`keep`).

## What the next pass should know

- The playlist (`keepSong` in `audio.ts`): a piece runs to its end and the shell's bag draws the next, during the scene too (`keep`); the opening with no bag is the seed's pool draw. No event on the field replaces a piece.
- Measure continuity, not only loudness: RMS per half second over the whole file, no window under -35 dBFS, before a bed is called mastered. `scripts/beds/arrange.py` still carries the route M1 shapes for the record; the installed set is generated, and a new bed is generated, measured, trimmed, mastered and encoded the same way.
- The Ghost pack gate names the seven beds by file and floors their size, as Vibe's does. The menu slot is empty on purpose; more beds are the next dogfood swarm's.
