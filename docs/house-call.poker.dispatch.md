...
> **PARKED by the Director, 2026-09-18.** The paper track stopped after slice 1 (`cabinet/house-call-s1` at `a924a16`, reviewed clean, not merged). He stopped it because a paper gate asked him to prove on printed cards what needs no proof, while the real question, whether the calibration layer feels right as an interactive addition, can only be judged in the browser. His decisions for the resume: the player sits against **model seats** picked with the same seat-picker format Ghost on the Menu uses; the game reads as poker first, in the browser, and the calibration layer rides on top. No paper play, no printing, no ticking. The next slice is a browser prototype Grok builds on the existing `packages/house-call` and serves on a port; chips pay like poker, the calibration readout stays off the felt. G46 (house pays) is his to reopen, since seated models mean other stacks at the table. Everything below is the record of the paper track, kept for the read and the receipts, not as a plan.
