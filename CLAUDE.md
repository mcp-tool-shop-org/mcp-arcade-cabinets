...
One cabinet in focus: **Ghost on the Menu** (replay shooter), published **v0.8.2**. House Call (turn-based calibration) was parked by the Director on 2026-09-10 because it was not fun; last state `152f548`. It comes back only when there is a design that plays. `tape-core` keeps the scoring rules for that day. Deferrals for the next pass live at the top of `HANDOFF.md` (translations, Comfy polish, `archivist.mp3`, health leftovers).
