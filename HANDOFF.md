...
**Pick up here (2026-09-17, `main` at `7dd375a`, released as v0.12.0; every push since `18e7fa5` is the same day's work and is listed below in order).** Three things landed in one line after Stage A, all verified together (1092 tests) and identity-scanned: **Stage B** of the dogfood swarm (wave 5, seven domains, `docs/dogfood-swarm.md`); **the answer to the Director's play notes** (`docs/ghost-attack.grok-consult.md`, the consult brief and what shipped from it: three line defects in the sim behind "only the boss attacks", the ship's shots capped per rung, four optional fire levers, the reader bot with a gun and a memory, the seat reader bar split, the play flowing into the next tape after the scene has held `NEXT_TAPE_S`); and **the music pass** (`docs/music.md`: fifteen beds rebuilt to about two minutes from their own stems and mastered, the loader settling on headers, the opening bed drawn by seed, a boss bed at once, and then, after his fourth listen, the Director chose a playlist for the music: the recorded beds are a playlist: a piece runs from its first bar to its last, the next is drawn without repeats until the set is spent, and no event on the field replaces a piece; a boss no longer brings its own music, and the scene between two tapes does not stop the piece; then, on his fifth listen, the whole set was found silent inside and replaced by seven pieces generated on Comfy Cloud with ACE-Step 1.5, each measured for continuity, trimmed, mastered to one level and installed, `docs/music.md`; the menu slot stays empty, and more pieces are generated the same way when wanted). The boss now waits for its wave's grid to be downed, or half the wave, before it takes the field (`BOSS_ENTRY_AT` in the sim), the Whisperer excepted. The tune that shipped is the Director's own heat, played and approved the same day (every class fires at every rung above the recorded one, on the way in, the first shot early, the ship capped; hardcore fast and wide on its one lamp; `docs/ghost-attack.grok-consult.md` records the consult's gentler recipe and why his read stood over it), and the fairness band's bars follow the tune, never the reverse. The built bundle is served from the music worktree on port 7791. The same evening the Director asked for the formation to reach the edges and for something to catch, and both shipped as levers (the formation's block drift per rung and its aimed fire; three fire drops, spread, rapid and pierce, by seeded draw; the caps up to four and three; the band's bot-survival bars retired with the reason, since no scripted bot lives under this heat and the human bar is his), and the endless slice E1 landed (`docs/ghost-endless.e1.md`; the sim, the levers and the band; E2 the call tool and the seat picker, E3 the shell and E4 the voice and film are next). **Standing on every update of this repo, the Director's rule: the lead adds and edits the agent's lines in `packages/ghost-on-the-menu/patterns/voice.json` (`docs/ghost-lines.md`: the voice, the gates, the pool sizes) until the pools feel diverse; every pool is a bag drawn without repeats.** The reactions authoring run landed (`docs/vibe-typer.reactions.md`: a `reaction` beside every one of the two hundred and forty-nine asks, Mistral writing, another family reading, twenty-nine rewritten by the lead after reading three levels as a player; the ghost pools grew by thirty-four in the same commit). **Stage C (humanization) landed** as waves 6 and 7 of the swarm (`docs/dogfood-swarm.md`): a hundred and seven fixes across the shell, both sims, the cabinet server and the launchers, verified together (1284 tests); the on-screen pass of the keyboard and screen-reader changes was read on the built bundle with Stage D's. **Stage D (visual polish) landed** as waves 8 and 9 (`docs/dogfood-swarm.md`): eighty-four fixes, the dark theme's contrast, the typing board's collapse, Ghost's field drawn at its hitboxes with captions off the trophy row, one help shape and one halt shape in the launchers, every argument described in the cabinet server's approval prompt, one label convention in the print, the authored pools normalized to one house style with no line reworded; verified together (1374 tests) and read on the built bundle in the browser pane. The health pass is whole. **The feature pass landed as polish** (waves 10 and 11, `docs/dogfood-swarm.md`): the audit's forty-eight proposals went to the Director ranked by product risk before any code, he took the cut at the coordinator's discretion for one day's wave and deferred the rest logged by id; twenty-four landed: both cabinets play on a phone (touch zones over Ghost's field, a hidden input under the typing editor, a glass line on each card), full screen survives the flow into the next tape (his note during the wave: full screen is the root now, not the field), the typing cabinet has a run code that replays and an end card that goes on, the hosted tier is opt-in and a Node under the floor is refused in words, the image answers help and version and stops on a signal, the `tapes` and `view` tools say what a client could not ask for, the fire seat backs off a dead daemon, and Ghost's sim says three things it computed and never said (the full column, a hull hit that lived, the fog landing; the blind pool's lines are the lead's). Verified together (1453 tests) and read on the built bundle at a phone's width. **v0.12.0 shipped the same evening** (the final test: the gate, the control plane's doctor, shipcheck at a hundred percent, both pack gates, the image built and smoked with its new help, version, tool lists and a stop in under half a second; the treatment: one version across the repo, the CHANGELOG's release header, both package READMEs brought to what npm shows, the root README's status and its seven translations, the handbook's getting-started, typing cabinet and cabinet-server pages, the site badge, the repo's description, the repo-knowledge entry; both tarballs, the image's app layer and the tree identity-scanned clean before the tag). Tag `v0.12.0`, `release.yml` published both packages to npm with provenance, the image is `ghcr.io/mcp-tool-shop-org/mcp-arcade-cabinets:0.12.0` for `linux/amd64` and `linux/arm64` (manifest list `sha256:1d882879d6ea8c4ef294fe0195ed7c5dd7ca1adbe52c71e21d43c0d799272518`), pulled back and smoked. Owed next: the twenty-four deferred proposals and the wave 11 seams left open, listed at the end of `docs/dogfood-swarm.md`; more beds for Ghost's playlist, generated the same way; `@mcptoolshop/vibe-typer@0.0.0` was deprecated as `placeholder` by the Director on 2026-09-17, which closes the row that had stood since 0.10.0. PyPI was asked about on the release and declined for this repo: the only Python here is the host-side voice worker; on the Director's word (2026-09-17) the voice loop becomes a tool of `mcp-arcade`, published through that repo's PyPI route, planned in its `HANDOFF.md`, and the cabinets point `VOICE_URL` at it when it ships. The Docker MCP Catalog still has no listing, and both are now in Docker's queue: the shooter's PR (docker/mcp-registry#5061, opened 2026-09-11, brought to the 0.12.0 pin on 2026-09-17, a review asked for in a comment) and the typing cabinet's (docker/mcp-registry#5141, opened 2026-09-17 after the Director asked why it was not already there; the rule now is that every distribution surface is a release step, `docs/cabinet-container.md` carries the per-release checklist). Both listings pass the registry's validator. The next game is MouseTrainer and Trace, under `E:/AI/prototypes/packages`, on the Director's word of 2026-09-17. Motif is the music slice after that. Every outside review to date is dispositioned in the run record (`docs/dogfood-swarm.md`).
